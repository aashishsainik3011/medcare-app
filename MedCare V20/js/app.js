...
const App = {
  ...
  updateDashboard() {
    ...
    // All-done celebration — fires when every scheduled dose for today is logged
    if (totalSlots > 0 && takenCount >= totalSlots && missedCount === 0 && upcoming === 0) {
      const celebratedKey = 'allDoneCelebrated_' + today;
      if (!DB.get(celebratedKey)) {
        DB.set(celebratedKey, true);
        setTimeout(() => this._showAllDoneCelebration(), 400);
      }
    }
  ...
  resetAddForm() {
    ...
    document.querySelectorAll('.dosage-chip').forEach(c => c.classList.remove('selected'));
...
{"name":"","dosage":"","type":"tablet","extra":"","notes":""}`;
  ...
  setDosageChip(btn) {
    document.getElementById('med-dosage').value = btn.textContent;
    document.querySelectorAll('.dosage-chip').forEach(c => c.classList.remove('selected'));
    btn.classList.add('selected');
  },

  closeAllDone() {
    const el = document.getElementById('all-done-overlay');
    if (el) el.classList.add('hidden');
    const conf = document.getElementById('all-done-confetti');
  ...
  _showAllDoneCelebration() {
    ...
    // Reuse existing confetti logic into the all-done confetti container
    const container = document.getElementById('all-done-confetti');
    if (!container) return;
    container.innerHTML = '';
    const colors = ['#FFFFFF','#FFD54F','#B8B0E8','#FFB300','#3A7D44','#81C784'];
    for (let i = 0; i < 60; i++) {
    ...
    setTimeout(() => this.closeAllDone(), 5000);
  },
